...
{
    if (!reader || !fs_text_check_path(path)) {
        return ESP_ERR_INVALID_ARG;
    }
    *reader = (fs_text_reader_t){0};
...
esp_err_t fs_text_reader_next(fs_text_reader_t *reader, char *buf, size_t buf_len, size_t *out_len)
{
    ...
    if (!reader || !reader->f || !buf || buf_len == 0) {
        return ESP_ERR_INVALID_ARG;
    }

    if (reader->offset >= reader->file_size) {
        return ESP_OK; // EOF reached previously
    }

    size_t max_chunk = buf_len;
    if (max_chunk > FS_TEXT_READER_CHUNK_BYTES) {
        max_chunk = FS_TEXT_READER_CHUNK_BYTES;
    }
    size_t remaining = reader->file_size - reader->offset;
    size_t to_read = remaining < max_chunk ? remaining : max_chunk;

    size_t n = fread(buf, 1, to_read, reader->f);
    if (n == 0) {
        if (ferror(reader->f)) {
            ESP_LOGE(TAG, "fread failed (errno=%d)", errno);
            return ESP_FAIL;
        }
        return ESP_OK; // EOF without error
    }
...
esp_err_t fs_text_reader_seek(fs_text_reader_t *reader, size_t offset)
{
    ...
    if (fseek(reader->f, (long)offset, SEEK_SET) != 0) {
        ESP_LOGE(TAG, "fseek failed (errno=%d)", errno);
        return ESP_FAIL;
    }
